...
5. **Matrix CI `[22, 24]` + audit moderate.** `MEJORAS.md` proponía `[20, 22]`, pero la realidad de la toolchain lo impide: `pnpm@11` (packageManager del repo) exige Node ≥ 22.13, y Node 20 alcanzó EOL en 2026-04. Se testean las dos líneas LTS mantenidas: 22 (LTS) y 24 (Active LTS). Nota para el runtime publicado: `engines >= 20` se revisará en un cambio propio si se decide acompañar Node 20 — el CLI consumido vía `npx` no usa pnpm. `pnpm audit --audit-level=moderate` como paso independiente — hoy el árbol está en 0 vulnerabilidades; si aparece una, el CI avisa en el PR.
